 ...
 */
public class Painter implements InputProcessor, Disposable {
	...
	private void drawTool(int x, int y) {
		switch (tool) {
			...
			case Eraser:
				for (Point point : brush.pairs) {
					erasePoint(x + point.x, y + point.y, point.color);
				}
				break;
		}
	}

	private void drawPoint(int x, int y, int softness) {
		if (x < 0 || x >= painterDelegate.getWidth() || y < 0 || y >= painterDelegate.getHeight())
			return;
		if (drawedPixels[x][y] < softness) {
			int currentColor = history.history.get(history.currentHistory).pixelData[x][y];
			int colorCode = painterDelegate.drawPoint(currentColor,brushColor, x, y, brushOpacity * (softness/255.0f));
			drawedPixels[x][y] = softness;
			affectedPixels.add(new Point(x, y, colorCode));
		}
	}

	private void erasePoint(int x, int y, int softness) {
		if (x < 0 || x >= painterDelegate.getWidth() || y < 0 || y >= painterDelegate.getHeight())
			return;
		if (drawedPixels[x][y] < softness) {
			int currentColor = history.history.get(history.currentHistory).pixelData[x][y];
			int colorCode = painterDelegate.erasePoint(currentColor,brushColor, x, y, brushOpacity * (softness/255.0f));
			drawedPixels[x][y] = softness;
			affectedPixels.add(new Point(x, y, colorCode));
		}
	}
	...
	public void output(FileHandle fh) {
		...
		history.recallHistory(painterDelegate, history.currentHistory); // ensure preview is not going to show up int he file..
