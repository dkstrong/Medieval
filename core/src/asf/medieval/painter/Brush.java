...
public class Brush {
	...
	private float hardEdge = 1;

	public Brush() {
		set(shape, radius, hardEdge);
	}
	...
	public float getHardEdge() {
		return hardEdge;
	}

	public void setHardEdge(float hardEdge) {
		set(shape, radius, hardEdge);
	}

	public void set(Shape shape, int radius, float softness) {
		if (shape == null) shape = Shape.Circle;
		if (radius < 0) radius = 0;
		if (softness < 0f) softness = 0;
		else if (softness > 1f) softness = 1f;

		this.shape = shape;
		this.radius = radius;
		this.hardEdge = softness;
		pairs.clear();
	...
	private void putPixel(int x, int y) {
		...
		if (hardEdge <1f) {
			float length = Vector2.len(x, y);
			float distFromEdge = radius - length;
			float opacity = UtMath.scalarLimitsInterpolation(distFromEdge, 0, radius* (1f-hardEdge), 0, 1f);
			opacityCode = Color.alpha(opacity);
		} else {
			opacityCode = Color.alpha(1.0f);
		}
		pairs.add(new Point(x, y, opacityCode));
		if (x < minX) minX = x;
